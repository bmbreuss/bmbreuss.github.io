...
function appendTableRows() {
      ...
      for (let i = 0; i < clubs.length; i++) {
            ...
            let cell0 = row.insertCell(0); // clubAbbrevlet 
            cell1 = row.insertCell(1); // avgDistlet 
            cell2 = row.insertCell(2); // minDistlet 
            cell3 = row.insertCell(3); // maxDistlet 
            cell4 = row.insertCell(4); // numOfShotslet 
            cell5 = row.insertCell(5); // ("+" button)
            let cell6 = row.insertCell(6); // clubName
            // right align only the cells that need to be right alignedcell0.className = "cmn_hidden"; // clubAbbrev
            ...
            cell5.innerHTML = "<button class='btn btn-success cmn_noPadding 
cmn_fullHeight' onclick='displayClubDistanceEntryForm(" + i + ");'>&nbsp;&nbsp;+&nbsp;&nbsp;</button>";
    ...
    function displayClubEntry() {
     ...
     document.getElementById('cmn_num').innerHTML = Math.round(clubs[clubRow][6]);
      ...
      function appendTapEntryButtons() {
           ...
           // set reasonable range for tapEntry buttonslet variation = 30;
